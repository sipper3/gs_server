...
public class XlsxView extends AbstractView {
	...
	protected void buildExcelDocument(Map<String, Object> model,
			SXSSFWorkbook wb, HttpServletRequest request, HttpServletResponse response) throws Exception {
	...
	protected SXSSFWorkbook getTemplateSource(String url, HttpServletRequest request) throws Exception {
		...
		SXSSFWorkbook xssfwb = new SXSSFWorkbook(new XSSFWorkbook(pkg));
    ...
    private int appendToExcelDocument(SXSSFWorkbook wb, List<ExcelMap> list, int begin) throws Exception {
		...
		for (int i = 0; i < list.size(); i++) {
			row = sheet.createRow(i + begin);
			map = list.get(i);
			cellIdx = 0;
			for (Object key : map.keySet()) {
				cell = creatCell(row, cellIdx, map.get(key));
    ...
    /**
     * 여러개의 sheet 생성이 필요한 경우 sheet를 생성(가져와)해서 엑셀을 만듬
     * @param wb SXSSFWorkbook
